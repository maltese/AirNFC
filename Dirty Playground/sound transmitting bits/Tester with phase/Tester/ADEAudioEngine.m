...
//    }
    ...
    for (int k = startFrequency; k < startFrequency + numberOfCarriers; k++) {
        ...
        // Compute phase shift using first carrier as pilot carrier
        if (k==startFrequency) {
            phaseShift = phases[0] - phase;
            if (phaseShift < M_PI) {
                phaseShift += 2*M_PI;
            } else if (phaseShift > M_PI) {
                phaseShift -= 2*M_PI;
            }
        }
        // Correct phase
        ...
        // Compute phase error
        float phaseError = phases[k-startFrequency] - phase;
//        if (phaseError < M_PI) {
//            phaseError += 2*M_PI;
//        } else if (phaseError > M_PI) {
//            phaseError -= 2*M_PI;
//        }
        
        if (fabsf(phaseError) > M_PI) {
            errorCount++;
        }
    }
...
//    printf("bit errors = %d\t%d\n", errorCount, numberOfCarriers);
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        [self.delegate audioEngineDidGetInterrupted:self hasData:data bits:errorCount];
    });
